...
import net.revenj.server.commands.search.SearchDomainObject.Argument
import net.revenj.server.{CommandResult, ReadOnlyServerCommand}

import scala.concurrent.Future
import scala.util.{Failure, Success, Try}

class SearchDomainObject(domainModel: DomainModel) extends ReadOnlyServerCommand {
    ...
    if (!arg.isSuccess) {
      ...
      if (!tryRepository.isSuccess) {
        ...
        if (trySpec.isFailure) {
          ...
          tryRepository.get.search(Option(trySpec.get), arg.get.Limit, arg.get.Offset).map { found =>
            val response = output.serializeRuntime(found)
            if (response.isSuccess) {
              CommandResult[TOutput](Some(response.get), s"Found ${found.length} items", 200)
            } else {
