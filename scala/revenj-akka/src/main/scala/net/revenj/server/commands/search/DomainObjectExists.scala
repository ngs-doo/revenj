...
import net.revenj.server.commands.search.DomainObjectExists.Argument
import net.revenj.server.{CommandResult, ReadOnlyServerCommand}

import scala.concurrent.Future
import scala.util.{Failure, Success, Try}

class DomainObjectExists(domainModel: DomainModel) extends ReadOnlyServerCommand {
    ...
    if (!arg.isSuccess) {
      ...
      if (!tryRepository.isSuccess) {
        ...
        if (trySpec.isFailure) {
          ...
          tryRepository.get.exists(Option(trySpec.get)).map { found =>
