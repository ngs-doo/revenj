...
class Create(domainModel: DomainModel) extends ServerCommand {
  ...
  override def execute[TInput, TOutput](
    ...
    if (!arg.isSuccess) {
      ...
      CommandResult.badRequest("Unable to find specified domain object: " + arg.get.Name)
      ...
      CommandResult.badRequest("Specified type is not an aggregate root: " + arg.get.Name)
    } else {
      val instance = input.deserializeRuntime[AggregateRoot](arg.get.Data, manifest.get)
      if (!instance.isSuccess) {
        CommandResult.badRequest("Error deserializing provided input for: " + arg.get.Name + ". Reason: " + instance.failed.get.getMessage)
      } else {
        val tryRepository = locator.resolve(classOf[PersistableRepository[AggregateRoot]], manifest.get)
        if (!tryRepository.isSuccess) {
          CommandResult.badRequest("Error resolving repository for: " + arg.get.Name + ". Reason: " + tryRepository.failed.get.getMessage)
        } else {
          ...
          tryRepository.get.insert(instance.get).map{ uri =>
            ...
            if (response.isSuccess) {
              new CommandResult[TOutput](Some(response.get), "Object created", 201)
            } else {
              new CommandResult[TOutput](None, response.failed.get.getMessage, 500)
            }
          }
        }
      }
