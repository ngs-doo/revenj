...
class Read(domainModel: DomainModel) extends ServerCommand {
  ...
  override def execute[TInput, TOutput](
    ...
    if (!arg.isSuccess) {
      ...
      CommandResult.badRequest("Specified type is not an identifiable: " + arg.get.Name)
    } else {
        ...
        tryRepository.get.find(arg.get.Uri).map {
          case Some(found) =>
            ...
            if (response.isSuccess) {
              new CommandResult[TOutput](Some(response.get), "Object found", 200)
            } else {
              new CommandResult[TOutput](None, response.failed.get.getMessage, 500)
            }
          case _ =>
            new CommandResult[TOutput](None, "Object not found", 404)
        }
      }
    }
  }
...
object Read {
  case class Argument(val Name: String, val Uri: String)
}