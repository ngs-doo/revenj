...
public class ReadOnlyDocumentRepository   implements java.io.Closeable, org.revenj.patterns.SearchableRepository<gen.model.binaries.ReadOnlyDocument> {
	
	
	
	public ReadOnlyDocumentRepository(
			 ...
			 final org.revenj.postgres.ObjectConverter<gen.model.binaries.ReadOnlyDocument> converter,
	...
	private final org.revenj.postgres.ObjectConverter<gen.model.binaries.ReadOnlyDocument> converter;
	...
	public ReadOnlyDocumentRepository(org.revenj.patterns.ServiceLocator locator) {
	...
	private java.util.List<gen.model.binaries.ReadOnlyDocument> readFromDb(java.sql.PreparedStatement statement, java.util.List<gen.model.binaries.ReadOnlyDocument> result) throws java.sql.SQLException, java.io.IOException {
	...
	@Override
	public java.util.List<gen.model.binaries.ReadOnlyDocument> search(org.revenj.patterns.Specification<gen.model.binaries.ReadOnlyDocument> specification, Integer limit, Integer offset) {
		...
		try (org.revenj.postgres.PostgresWriter pgWriter = org.revenj.postgres.PostgresWriter.create()) {
			...
			if (specification == null) {
				sql = "SELECT r FROM (SELECT \"ID\", name from binaries.\"Document\") r";
			} 
			else {
				org.revenj.patterns.Query<gen.model.binaries.ReadOnlyDocument> query = query(specification);
				if (offset != null) {
	...
	@Override
	public long count(org.revenj.patterns.Specification<gen.model.binaries.ReadOnlyDocument> specification) {
		...
		try (org.revenj.postgres.PostgresWriter pgWriter = org.revenj.postgres.PostgresWriter.create()) {
			String sql;
			if (specification == null) {
				sql = "SELECT COUNT(*) FROM (SELECT \"ID\", name from binaries.\"Document\") r";
			} 
			else {
				try {
	...
	@Override
	public boolean exists(org.revenj.patterns.Specification<gen.model.binaries.ReadOnlyDocument> specification) {
		...
		try (org.revenj.postgres.PostgresWriter pgWriter = org.revenj.postgres.PostgresWriter.create()) {
			String sql = null;
			if (specification == null) {
				sql = "SELECT exists(SELECT * FROM (SELECT \"ID\", name from binaries.\"Document\") r";
			} 
			else {
				try {
