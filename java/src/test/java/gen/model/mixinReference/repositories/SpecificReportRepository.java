...
public class SpecificReportRepository   implements org.revenj.patterns.Repository<gen.model.mixinReference.SpecificReport>, org.revenj.patterns.PersistableRepository<gen.model.mixinReference.SpecificReport> {
	...
	@Override
	public long count(java.util.Optional<org.revenj.patterns.Specification<gen.model.mixinReference.SpecificReport>> filter) {
		if (filter == null || filter.orElse(null) == null) {
			try (java.sql.PreparedStatement statement = connection.prepareStatement("SELECT COUNT(*) FROM \"mixinReference\".\"SpecificReport_entity\" r");
	...
	@Override
	public boolean exists(java.util.Optional<org.revenj.patterns.Specification<gen.model.mixinReference.SpecificReport>> filter) {
		if (filter == null || filter.orElse(null) == null) {
			try (java.sql.PreparedStatement statement = connection.prepareStatement("SELECT exists(SELECT * FROM \"mixinReference\".\"SpecificReport_entity\" r)");
				java.sql.ResultSet rs = statement.executeQuery()) {
				rs.next();
