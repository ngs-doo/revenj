...
namespace DSL
{
	public static class Core
	{
		...
		public static void ConfigurePatterns(this IObjectFactoryBuilder builder, Func<IObjectFactory, IEnumerable<Assembly>> getDomainModels)
		{
			...
			builder.RegisterType<EventSource, IEventSource>(InstanceScope.Context);
			builder.RegisterType<EventStore, IEventStore, IDomainEventStore>(InstanceScope.Context);
			builder.RegisterType<GlobalEventStore>(InstanceScope.Context);
			builder.RegisterType(typeof(SingleDomainEventSource<>), InstanceScope.Context, true, typeof(IEventSource<>));
