...
namespace LitS3
{
	...
	public class ListObjectsArgs
	{
		...
		internal string ToQueryString()
		{
			...
			{ builder.Append(sep).Append("prefix=").Append(Uri.EscapeUriString(Prefix)); sep = '&'; }

			if (!string.IsNullOrEmpty(Marker))
			{ builder.Append(sep).Append("marker=").Append(Uri.EscapeUriString(Marker)); sep = '&'; }

			if (!string.IsNullOrEmpty(Delimiter))
			{ builder.Append(sep).Append("delimiter=").Append(Uri.EscapeUriString(Delimiter)); sep = '&'; }
