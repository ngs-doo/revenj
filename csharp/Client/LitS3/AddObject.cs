...
namespace LitS3
{
	...
	public class AddObjectRequest : S3Request<AddObjectResponse>
	{
		...
		public long ContentLength
		{
			...
			{
				long cl;
				long.TryParse(WebRequest.Headers["Content-Length"], out cl);
				return cl;
			}
			set { WebRequest.Headers["Content-Length"] = value.ToString(); contentLengthWasSet = true; }
		}
		...
		protected override void Authorize()
		{
			...
			foreach (var kv in metadata)
				WebRequest.Headers[S3Headers.MetadataPrefix + kv.Key] = string.Join(";", kv.Value);
