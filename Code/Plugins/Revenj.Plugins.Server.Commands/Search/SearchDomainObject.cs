...
" + CommandResult<TOutput>.ConvertToString(CreateExampleArgument(output)));
	...
	public static class SearchDomainObjectHelper
	{
		internal class Repository<TData, TFormat> : IQueryableRepository<TData>
		{
			private readonly IServiceLocator Locator;
			...
			public static IQueryable<TData> Query<TCondition>(
				IServiceLocator locator,
				SearchDomainObject searchCommand,
				ISerialization<TFormat> serializer,
				ISpecification<TCondition> specification)
			{
				var result =
					searchCommand.Execute(
						...
						serializer,
						serializer.Serialize(
							new SearchDomainObject.Argument<TFormat>
							{
								Name = typeof(TData).FullName,
								SpecificationName = specification != null ? specification.GetType().FullName : null,
								Specification = specification != null ? serializer.Serialize(specification) : default(TFormat),
								Limit = int.MaxValue
							}));
				return serializer.Deserialize<TFormat, TData[]>(result.Data, locator).AsQueryable();
			}

			public static TData[] Search<TCondition>(
				IServiceLocator locator,
				SearchDomainObject searchCommand,
				ISerialization<TFormat> serializer,
				ISpecification<TCondition> specification,
			...
			{
				return Query(Locator, SearchCommand, Serializer, specification);
			}

			public TData[] Search<TCondition>(ISpecification<TCondition> specification, int? limit, int? offset)
			{
				return Search(Locator, SearchCommand, Serializer, specification, limit, offset);
			}
		}

		public static IQueryableRepository<TData> CreateQueryableRepository<TData, TFormat>(this IServiceLocator locator)
		{
			return
