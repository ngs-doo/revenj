...
{
	[OracleCustomTypeMapping("-Revenj-.BOOL_ARR")]
	public class BoolArrayConverter : IOracleCustomType, IOracleCustomTypeFactory, IOracleArrayTypeFactory, INullable, IOracleTypeConverter, IOracleVarrayConverter
	{
		...
		public string ToStringVarray(IEnumerable value)
		{
			...
			return "new \"-Revenj-\".STRING_ARR(" + string.Join(",", values.Select(it => ToString(it))) + ")";
		}
		...
		{
			return new OracleParameter { OracleDbType = OracleDbType.Array, Value = Create(value.Cast<bool?>()), UdtTypeName = "-Revenj-.STRING_ARR" };
		}
	}
}
