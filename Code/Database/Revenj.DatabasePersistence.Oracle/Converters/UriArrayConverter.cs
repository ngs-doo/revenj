...
{
	[OracleCustomTypeMapping("-Revenj-.URI_ARR")]
	public class UriArrayConverter : IOracleCustomType, IOracleCustomTypeFactory, IOracleArrayTypeFactory, INullable, IOracleTypeConverter, IOracleVarrayConverter
	{
		...
		public static UriArrayConverter Create(IEnumerable<string> collection)
		{
			return new UriArrayConverter { Value = collection != null ? collection.ToArray() : null };
		}
		...
		public string ToStringVarray(IEnumerable value)
		{
			...
			return "new \"-Revenj-\".URI_ARR(" + string.Join(",", values.Select(it => ToString(it))) + ")";
		}
		...
		{
			return new OracleParameter { OracleDbType = OracleDbType.Array, Value = Create(value.Cast<string>()), UdtTypeName = "-Revenj-.URI_ARR" };
		}
	}
}
