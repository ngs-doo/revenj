...
namespace NGS.Utility
{
	...
	/// Default temporary folder is created at NGS/Temp subfolder in system temporary folder
	...
	public static class TemporaryResources
	{
		...
		static TemporaryResources()
		{
			...
			TempPath = !string.IsNullOrEmpty(tp) && Directory.Exists(tp) ? tp : Path.Combine(Path.GetTempPath(), "NGS", "Temp");
		}
		/// <summary>
